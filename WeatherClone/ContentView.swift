...
struct ContentView: View {
	...
	let backgroundTopStops: [Gradient.Stop] = [
		.init(color: .midnightStart, location: 0.0),
	...
	let backgroundBottomStops: [Gradient.Stop] = [
		.init(color: .midnightEnd, location: 0.0),
	...
	let cloudTopStops: [Gradient.Stop] = [
		.init(color: .darkCloudStart, location: 0.0),
	...
	let cloudBottomStops: [Gradient.Stop] = [
		.init(color: .darkCloudEnd, location: 0.0),
	...
	var starOpacity: Double {
		...
		let opacity = color.getComponents().alpha
		return opacity
	}
		...
		.safeAreaInset(edge: .bottom) {
			VStack {
				...
				Picker("Precipitation", selection: $stormType) {
					ForEach(Storm.Contents.allCases, id: \.self) { type in
